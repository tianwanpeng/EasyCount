...
public class ExprNodeNewDefineDesc extends ExprNodeDesc implements Serializable {
	...
	public ExprNodeNewDefineDesc(final ArrayList<Var> vars) {
	...
	public boolean isSame(final Object o) {
		...
		final ArrayList<Var> vars1 = (ArrayList<Var>) o;
		if (this.vars.size() != vars1.size()) {
			return false;
		}
		for (int i = 0; i < this.vars.size(); i++) {
			if (!this.vars.get(i).equals(vars1.get(i))) {
				return false;
			}
		}
		return true;
