package com.tencent.easycount.util.bloom;
...
public class BloomFilter<T> implements Cloneable, Serializable {
	private static final long serialVersionUID = -3516593946171921712L;
	 ...
	 * cryptographic hash function (e.g. MD5, SHA-512, default is MD5) which can
	 * be set using {@link #setCryptographicHashFunction(String)}. It slices the
	 * digest in bit ranges of x with 2^x > m and does rejection sampling for
	 * each slice. It is fast and very well distributed.</li>
	...
	public int valueSize() {
		double N = this.size();
		double k = this.getK();
		double X = this.getBitSet().cardinality();
		return (int) ((-1) * N / k * Math.log(1 - X / N));
	}
