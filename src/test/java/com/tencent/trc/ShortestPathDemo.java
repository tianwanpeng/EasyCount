...
public class ShortestPathDemo extends JPanel {
			...
			public void paint(Graphics g) {
				...
				for (Iterator iter = layout.getGraph().getEdges().iterator(); iter
					...
					if (isBlessed(e)) {
						...
						pr.paintEdge(g2d, e, (int) p1.getX(), (int) p1.getY(),
	...
	private Component getSelectionBox(final boolean from) {
		...
		for (Iterator iter = mGraph.getVertices().iterator(); iter.hasNext();) {
	...
	static Graph getGraph() {
		...
		Graph g = (Graph) new EppsteinPowerLawGenerator(26, 50, 50)
				.generateGraph();
