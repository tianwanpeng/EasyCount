...
public class TestSouthernWomenBipartite extends JPanel {
	...
	void foldAndDisplay(KPartiteGraph bpgx) {
		...
		bipartiteGraphDraw = new LocalGraphDraw(f, new CircleLayout(),
				new SpringLayout(), vr, er, d, true);
	...
	private List getListOfDates() throws ParseException {
		...
		Set dates_part = PredicateUtils.getVertices(bpg,
				BipartiteGraphReader.PART_B);
		for (Iterator iter = dates_part.iterator();
	 ...
	 * @param bpg
	 */
	protected static void dumpNames(KPartiteGraph bpg, StringLabeller women,
			StringLabeller dates) {
		for (Iterator iter = PredicateUtils.getVertices(bpg,
				BipartiteGraphReader.PART_A).iterator(); iter.hasNext();) {
			Vertex bpv = (Vertex) iter.next();
			System.out.println(women.getLabel(bpv) + " ("
					+ bpv.getNeighbors().size());
			for (Iterator iterator = bpv.getNeighbors().iterator(); iterator
					.hasNext();) {
	...
	private LocalGraphDraw createVisualization() {
		...
		Dimension d = new Dimension(450, 450);
		LocalGraphDraw lgd = new LocalGraphDraw(bpg, new RandomLayout(),
				new KKLayout(), wellKnownRenderer, er, d, true);
	...
	public class RemoveAndRefold implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			for (int i = 0; i < selected.length; i++) {
				...
				if (v2 == null) {
					System.out.println("No original vertex with label "
							+ selected[i]);
				}
